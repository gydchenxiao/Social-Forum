...
import io.college.modules.admin.service.*;
import io.college.modules.app.dao.FollowDao;
import io.college.modules.app.entity.FollowEntity;
import io.college.modules.app.param.*;
import io.college.modules.app.service.FollowService;
...
import io.college.modules.admin.dao.AppUserDao;
...
public class AppUserServiceImpl extends ServiceImpl<AppUserDao, AppUserEntity> implements AppUserService {
    ...
    public void ban(Integer id) {
        ...
        if (status.equals(Constant.USER_BANNER)) {
            throw new collegeException("该用户已被禁用");
        }
        this.lambdaUpdate()
    ...
    public void openBan(Integer id) {
        ...
        if (status.equals(Constant.USER_NORMAL)) {
            throw new collegeException("该用户已解除禁用");
        }
        boolean update = this.lambdaUpdate()
                ...
                .update();
        if(!update){
            throw new collegeException("解除失败");
        }
    }
    ...
    public Integer smsLogin(SmsLoginForm form, HttpServletRequest request) {
        ...
        if (io.college.common.utils.ObjectUtil.isEmpty(s)) {
            throw new collegeException("请先发送验证码");
        }
        if (!s.equals(form.getCode())) {
            throw new collegeException("验证码错误");
        }
        if (ObjectUtil.isNotNull(appUserEntity)) {
            //登录
            if (appUserEntity.getStatus().equals(Constant.USER_BANNER)) {
                throw new collegeException("该账户已被禁用");
            }
            ...
            if(ObjectUtil.isNull(user)){
                throw new collegeException("注册失败");
            }
            //其他业务处理
            return user.getUid();
        }
...
//        }
    ...
    public AppUserInfoResponse findUserInfoById(Integer uid, AppUserEntity user) {
        ...
        if(ObjectUtil.isNull(userEntity)){
            throw new collegeException("用户不存在");
        }
    ...
    public Integer miniWxLogin(WxLoginForm form) {
        ...
        if(io.college.common.utils.ObjectUtil.isEmpty(openId)){
            throw new collegeException("请正确配置appId和密钥");
        }
    ...
    private String getOpenId(String code){
        SystemEntity system = systemService.lambdaQuery()
                ...
                .one();
        if(system == null){
            throw new collegeException("后台配置项不存在");
        }
